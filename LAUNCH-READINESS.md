...
| Frontend | ✅ Static `frontend/index.html` (live reads + wallet-backed writes) on esm.sh `2.0.0-rc.1` + `studioDevnet` + current contract `0x278C...` + explorer-studio-dev + hash-bound evidence rows; public build verified 2026-09-16 at `https://provedowngen.vercel.app/` (`/`, `/verify`, `/proof` 200, unknown 404) | `npm --prefix frontend test`, `npm --prefix frontend run build`, dynamic Chromium smoke |
