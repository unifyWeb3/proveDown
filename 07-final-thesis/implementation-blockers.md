...
> **Historical / superseded baseline.** This blocker table captures the 2026-09-03 pre-build state. The frontend, Worker, and Studio contract gates are now implemented and verified. Keep this file as an audit trail; use [`research/CODEX_FULL_AUDIT.md`](../research/CODEX_FULL_AUDIT.md) and [`LAUNCH-READINESS.md`](../LAUNCH-READINESS.md) for the active blockers. The `appeal_attestation` row is a documentation mismatch, not authorization to add an appeal method to the MVP.
...
| **P1-4** | Same private key for both chains (redacted value identical) + `*:Zone.Identifier` leak already ignored | `.env.local:6,19` + `.gitignore` | Rotate Base key via `cast wallet new` (if `cast` available) or keep for testnet but add comment `# testnet only, rotate before mainnet` | `cast` or manual | redacted-prefix scan over tracked files → 0 after rotate |
